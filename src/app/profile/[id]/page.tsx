...
import { 
  Calendar, MapPin, Briefcase, Building2, 
  Clock, UserCircle, Phone, Mail, ArrowLeft, AlertCircle 
} from 'lucide-react';
...
export default function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3001/v1/users/user/${username}`);

        if (!response.ok) {
          if(response.status === 404) throw new Error("Usuario no encontrado en el sistema.");
  ...
  if (loading) return (
    <div className="flex flex-col justify-center items-center min-h-screen">
      ...
      <p className="mt-4 text-gray-500 font-bold">Consultando API externa...</p>
    </div>
  );
  ...
  if (error || !profileData) return (
    <div className="flex justify-center items-center min-h-screen p-6">
      <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-red-100 text-center max-w-md">
        ...
        <h2 className="text-2xl font-black text-gray-900 mb-2">Error de Perfil</h2>
        <p className="text-gray-500 mb-6">{error}</p>
        <button onClick={() => router.back()} className="w-full py-3 bg-gray-900 text-white rounded-2xl font-bold">Volver</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8 animate-fadeIn">
      <button onClick={() => router.back()} className="flex items-center text-gray-400 hover:text-blue-600 font-black text-xs tracking-[0.2em]">
        ...
        <div className="px-10 pb-10">
          <div className="relative flex justify-between items-end -mt-20 mb-8">
            <Avatar 
              ...
              fallbackLetter={profileData.nombre?.charAt(0)}
            />
            <div className="flex flex-col items-end gap-3">
              ...
              <span className="text-xs font-bold text-gray-400">ID EXTERNO: {profileData.id}</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            <InfoItem icon={<Building2 />} label="Sucursal" value={emp?.sucursalActiva?.nombre} subValue={emp?.sucursalActiva?.clave} />
            <InfoItem icon={<Briefcase />} label="Departamento" value={emp?.departamento?.nombre} />
            <InfoItem icon={<Clock />} label="Antigüedad" value={calcularAntiguedad(emp?.fechaIngreso)} />
            <InfoItem icon={<MapPin />} label="Ubicación" value={`${emp?.municipio}, ${emp?.estado}`} />
          </div>
        </div>

        {/* CONTACTO */}
        <div className="bg-gray-900 p-10 rounded-[3rem] text-white">
          <h2 className="text-blue-400 font-black uppercase tracking-widest mb-10 text-center text-sm">Información Personal</h2>
          <div className="space-y-8">
            <ContactItem icon={<Phone />} label="Celular" value={emp?.celular || 'No disponible'} />
            <ContactItem icon={<Mail />} label="Email" value={emp?.email || 'Sin correo registrado'} />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-start gap-4">
      ...
      <div>
        ...
        {subValue && <p className="text-xs text-blue-500 font-bold">{subValue}</p>}
      </div>
    </div>
  );
}
